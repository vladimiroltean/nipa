...
prep_config() {
  make CC="$cc" O=$output_dir ARCH=$arch allmodconfig
  ./scripts/config --file $output_dir/.config --disable gcc_plugins
  ./scripts/config --file $output_dir/.config --disable werror
...
then
    ...
    prep_config
    make CC="$cc" O=$output_dir ARCH=$arch $build_flags
fi
...
prep_config
make CC="$cc" O=$output_dir ARCH=$arch $build_flags 2> >(tee $tmpfile_o >&2)
...
prep_config
make CC="$cc" O=$output_dir ARCH=$arch $build_flags 2> >(tee $tmpfile_n >&2) || rc=1
